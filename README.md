...
<h1> Multiple Angles to this problem </h1>
<li> Reasoning Module to alleviate the lack of data </li>
<li> Reducing the bias in the model due to lack of statistical evidence </li>
<li> How do create model that help businesses to objectively evaluate cases with lack of data through ground up reasoning </li>
<li> How to extrapolate from small volume of data without having to incur cost of data collection and training </li>
<li> How to implicitly evaluate the lifestyle of people from a single action </li>

<h1> Lack of Statistical Evidence </h1>
It's really hard to extrapolate when the statisical evidence for an event isn't there. Imagine a company is successful and wants to go global. Every country is an entire new landscape different set of people and behavior patterns. People around the world driven by variety of parameters like culture, climate, state of the economy, GDP growth, unemployment, religion, etc to name some large parameters. On a smaller note behavior patterns would rely on day to day activity and how does the companies solution would benefit the people. Whether benefit is going to be experienced as time saving factor, cost saving factor, productivity factor, leisure factor, convienance factor etc. 

We would need model that could recognize patterns from companies that flourish in those country to model for growth prospectives. Psychology creates a stochastic environment by showcasing people's behavior to be different and can vary due to lot of factor. The famous quote from Newton would strike us "I can calculate the motion of objects but not the emotional behavior of people. If we look beyond psychology and look at praxelogy which is a parent class of psychology. It defines every action humans take is purposeful and each action  taken would depend on individual perception of how much does it alleviate the existing situation. Whether to cook and eat to alleviate the little hunger now or wait for some more time before to take any action. If we evaluate behavior patterns from a praxeology stand point then the whole model is determinisitic because we only need to figure out incentive that would make people transition from one state to another. 
...
<p>Warehouse technology requires placing inventory in warehouses that is well optimized for space usage. Products have to allocated a specific location depending on number of times they have to retrieved for fulfillment. It's multi-objective optimization here the constrain is not the space allocation alone but also time to retrieve the product & energy expended in retrieving them. Also products should be handled with care to avoid damages and spillages. There are determinisitic approaches to finding optimal solution in the above scenarios using evolutionary algorithms. 
